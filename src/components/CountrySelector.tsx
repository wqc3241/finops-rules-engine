  ...
  return <Popover>
      ...
      <PopoverContent className="w-80 p-0 bg-white border border-gray-200 shadow-lg" align="end">
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          return <div key={region} className="p-2">
                <div className="px-2 py-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  {region}
                </div>
                <div className="space-y-1">
                  {countries.map(country => <button key={country.code} onClick={() => setSelectedCountry(country)} className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-colors ${selectedCountry.code === country.code ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
                      <span className="text-base">{country.flag}</span>
                      <div className="flex-1 text-left">
                        <div className="font-medium">{country.name}</div>
                        <div className="text-xs text-gray-500">{country.code} • {country.currency}</div>
                      </div>
                      {selectedCountry.code === country.code && <div className="w-2 h-2 bg-blue-600 rounded-full"></div>}
                    </button>)}
                </div>
              </div>;
        })}
