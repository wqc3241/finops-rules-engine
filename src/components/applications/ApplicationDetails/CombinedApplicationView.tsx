...
const CombinedApplicationView: React.FC<CombinedApplicationViewProps> = ({
  ...
  const [reapplyEnabled, setReapplyEnabled] = useState(applicationFullDetails.details.reapplyEnabled || false);
  ...
  const handleSectionClick = (section: keyof typeof expandedSections) => (e: React.MouseEvent) => {
    ...
    if (target.closest('button') || target.closest('a') || target.closest('[role="button"]') || target.closest('input') || target.closest('select') || target.closest('.card-container') || target.closest('[data-lender-card]') || target.closest('[data-interactive]')) {
      return;
    }
  ...
  const handleReapplyToggle = (enabled: boolean) => {
    ...
    toast({
      ...
      description: enabled ? "Customer can now request to reapply for this order." : "Customer can no longer request to reapply for this order."
    });
  };

  // Check if reapply can be enabled based on current status
  const canEnableReapply = canReapply(applicationFullDetails.details);

  // Ensure applicationType is properly typed as 'Lease' | 'Loan'
  const applicationType = applicationFullDetails.details.type === 'Lease' || applicationFullDetails.details.type === 'Loan' ? applicationFullDetails.details.type as 'Lease' | 'Loan' : 'Loan'; // fallback to 'Loan' if type is not recognized
      ...
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2">
            ...
            <Switch id="reapply-toggle" checked={reapplyEnabled} onCheckedChange={handleReapplyToggle} disabled={!canEnableReapply} />
          </div>
          {!canEnableReapply}
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg">
            ...
            <CollapsibleContent>
              <div className="px-3 pb-3">
                <ApplicationData applicantInfo={applicationFullDetails.applicantInfo} coApplicantInfo={applicationFullDetails.coApplicantInfo} vehicleData={applicationFullDetails.vehicleData} appDtReferences={applicationFullDetails.appDtReferences} />
              </div>
            </CollapsibleContent>
          </div>
        </Collapsible>
