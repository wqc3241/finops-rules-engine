...
import { AlertCircle, CheckCircle2, Loader2, Database, Car, Users } from 'lucide-react';
import Navbar from '@/components/Navbar';
import Sidebar from '@/components/Sidebar';
import { populateVehicleData } from '@/utils/populateVehicleData';
import { populateApplicationData } from '@/utils/populateApplicationData';
...
const DataMigration = () => {
  ...
  const handlePopulateApplicationData = async () => {
    setStatus('migrating');
    setMessage('Generating and inserting applicant info and order details... This may take a moment.');

    try {
      const result = await populateApplicationData();
      
      if (result.success) {
        setStatus('success');
        setMessage(result.message || 'Application data populated successfully!');
      } else {
        setStatus('error');
        setMessage(result.message || 'Failed to populate application data');
      }
    } catch (error: any) {
      setStatus('error');
      setMessage(`Population failed: ${error.message}`);
      console.error('Application data population error:', error);
    }
  };
                ...
                <div className="border-t pt-4 space-y-4">
                  ...
                  <div>
                    <h3 className="font-semibold mb-3 flex items-center gap-2">
                      ...
                      Application Data Population
                    </h3>
                    <p className="text-sm text-muted-foreground mb-3">
                      Generate and insert complete applicant information and order details for all applications (primary + co-applicants, trade-ins, sale data).
                    </p>
                    <Button onClick={handlePopulateApplicationData} size="lg" variant="default">
