-- Upsert financial_program_configs row for NL_GDE26_0825_1 based on bulletin_pricing
-- Build template_metadata from existing bulletin_pricing rows for the program
WITH bp AS (
  SELECT
...
-- Update if the program exists and there are rows in bulletin_pricing
...
-- Insert if it does not exist yet and there are rows in bulletin_pricing
